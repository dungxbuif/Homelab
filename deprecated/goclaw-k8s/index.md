---
type: Reference
title: "Code Index: goclaw-k8s"
description: "Aggregated code index for goclaw-k8s folder"
timestamp: 2026-07-03T15:11:00Z
---

# Code Index: goclaw-k8s

> This index aggregates code files in the [[goclaw-k8s/]] directory.
...
spec:
  project: default
  source:
    repoURL: 'https://github.com/dungxbuif/Homelab.git'
    targetRevision: HEAD
    path: iac/k8s/goclaw
  destination:
      ...
      containers:
        - name: goclaw
          ...
          env:
            ...
            - name: GOCLAW_SKILLS_DIR
              value: "/app/data/skills"
            - name: GOCLAW_ALLOW_INSECURE_NO_AUTH
              value: "0"
...
kind: IngressRoute
metadata:
  name: goclaw-ingressroute
  namespace: goclaw
spec:
  entryPoints:
    - web
  routes:
    - match: Host(`goclaw.dungxbuif.com`)
      kind: Rule
      services:
        - name: goclaw
          port: 18790
