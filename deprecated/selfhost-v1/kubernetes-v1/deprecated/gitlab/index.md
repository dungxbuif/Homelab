...
## [gitlab-minio-creds.yaml](./gitlab-minio-creds.yaml)
...
stringData:
  connection: |
    s3:
      ...
      accesskey: gitlab
      secretkey: rdh-FRN3qjh4pwv_ztp
      region: us-east-1
      regionendpoint: "http://minio-service.minio-system.svc:9000"
      v4auth: true
```
...
global:
  ...
  hosts:
    ...
    minio:
      enabled: false
    registry:
      name: gitlab-registry.dungxbuif.com
    tls: {}
    smartcard: {}
    kas: {}
    pages: {}
  ...
  gitaly:
    ...
    tls:
      ...
      # secretName:
    persistence:
      size: 2Gi # Reduced from 20Gi to 8Gi
  ...
  registry:
    ...
    notifications: {}
      ...
      #         key: password
      # events: {}
    storage: # Configure registry storage to use external MinIO
      driver: s3
      s3:
        bucket: registry # Bucket name in your MinIO for the registry
        region: us-east-1 # MinIO region (can usually be anything)
        accesskeySecret:
          name: gitlab-minio-creds
          key: accesskey
        secretkeySecret:
          name: gitlab-minio-creds
          key: secretkey
        host: minio-service.minio-system.svc:9000 # Internal K8s service hostname and port for MinIO API
        protocol: http # Protocol for MinIO API endpoint
...
redis:
  ...
  master:
    ...
    #     memory: 300Mi
  replica:
    replicaCount: 0  # Disable Redis replicas entirely
...
registry:
  ...
  #     memory: 200Mi
  persistence:
    size: 5Gi  # Default or less if needed
...
gitlab:
  ...
  webservice:
    minReplicas: 1
    maxReplicas: 1  # Reduced from default 2
    # resources:
    #   requests:
  ...
  gitaly:
    ...
    #     memory: 250Mi   # Reduced memory request
    #   limits:
    #     cpu: 500m       # Added CPU limit
    #     memory: 500Mi   # Added memory limit
    # livenessProbe:
    #   enabled: false # Disable liveness probe
